...
class Parser(object):
    ...
    class GroupItem(object):
        ...
        def __repr__(self):
            return "Group `%s'\n FileRefs: %s\n Sub-Groups: %s" %\
                   (self.name, self.filerefs, self.groups)
    ...
    class FileRefItem(object):
        ...
        def __repr__(self):
            return u"File at %s" % self.path

    def _parse_fileref(self, fileRefTag):
        location = fileRefTag.attrib.get('location', None)
        if not location:
            raise Parser.Error('FileRef should have "location" attribute! %s'
                               % fileRefTag.attrib)
    ...
    def parse(self, stream):
        ...
        if workspaceVersion != '1.0':
            raise Parser.Error('Supported only Workspace version 1.0.'
                               ' Provided: %s' % workspaceVersion)
